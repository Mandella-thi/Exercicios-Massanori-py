...
def preposicao(b):
    cont=0
    for x in b:
        if x[0] in ['z','m', 'b'] and x[-1] not in ['z', 'b','m']:
            cont= cont +1
    return cont
print(f' O número de preposições é: {preposicao(txtB)}')
def verbo(b):
    cont=0
    for x in b:
        if len(x)==7 and x[-1] not in ['z','b','m']:
            cont = cont +1
    return cont
def verbo_prim(b):
    cont=0
    for x in b:
        if len(x)==7 and x[-1] not in ['z','b','m'] and x[0] not in['z','b','m']:
            cont= cont +1

    return cont

print(f'O número de verbos é:  {verbo(txtA)}')

print(f'O número de verbos na primeira pessoa são:{verbo_prim(txtB)}')
